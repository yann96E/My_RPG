                                                  ...
                                                  int const offset)
{
    ...
    while (path->next) {
        ...
        for (file_path_t *i = path->next; i; NEXT(i)) {
            biggest = ((strcmp_crt_number(biggest->path + offset,
                          ...
                          bool const check_extension)
{
    ...
    while ((cur_file = readdir(dir)) != NULL) {
        if (*cur_file->d_name == '.' ||
            ...
            continue;
        path = new_sprite((check_extension == false) ?
                          (my_strcat_end(folder_path, cur_file->d_name, '/')) :
                          (my_strcat_nf(folder_path, cur_file->d_name)), &path);
        if (!path) return (NULL);
        ++*len;
    }
    closedir(dir);
